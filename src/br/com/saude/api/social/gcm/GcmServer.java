...
public abstract class GcmServer {  
  ...
  public GcmServer(String apiKey, String senderId, String serviceName) {
    ...
    smackCcsClient = new SmackCcsClient(apiKey, username, serviceName, Constants.GCM_HOST, Constants.GCM_CCS_PORT);
    System.out.println("smackCcsClient("+apiKey+", "+username+", "+serviceName+", "+Constants.GCM_HOST+", "+Constants.GCM_CCS_PORT+")\n");
    ...
    stanzaListener = new StanzaListener() {
      ...
      public synchronized void processPacket(Stanza packet) throws SmackException.NotConnectedException {
        ...
        GcmPacketExtension packetExtension =
            (GcmPacketExtension) packet.getExtension(Constants.GCM_NAMESPACE);
  ...
  private String wrapWithXML(String payload) {
    String msg = String.format("<message><%s xmlns=\"%s\">%s</%s></message>",
    		Constants.GCM_ELEMENT_NAME, Constants.GCM_NAMESPACE, payload, Constants.GCM_ELEMENT_NAME);
